...
export const MobileNavigation = ({ menuItems, connecting }: MobileNavigationProps) => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 flex justify-around items-center bg-western-wood/90 backdrop-blur-sm shadow-lg py-2 px-2 z-50">
      {menuItems.map(item => (
        <Link 
          key={item.path} 
          to={item.path} 
      ...
      {!isConnected && (
        <Button 
          ...
          disabled={connecting} 
          className="flex flex-col items-center justify-center p-1.5 h-auto text-western-sand hover:text-western-parchment"
        >
          <Wallet className="h-3.5 w-3.5" />
          <span className="text-[10px] mt-0.5 font-western">
            {connecting ? "..." : "Connect"}
          </span>
        </Button>
      )}
