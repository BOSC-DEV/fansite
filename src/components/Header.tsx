...
export const Header = () => {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out wood-texture", isScrolled ? "py-3 shadow-md" : "py-5")}>
      <div className="container mx-auto md:px-6 flex items-center justify-between px-[2px]">
        ...
        <div className="flex items-center space-x-4">
          ...
          {isMobile && <div className="md:hidden flex items-center">
              ...
              <div className="fixed bottom-0 left-0 right-0 flex justify-around items-center bg-western-wood/90 backdrop-blur-sm shadow-lg py-3 px-2 z-50">
                {menuItems.map(item => <Link key={item.path} to={item.path} className={cn("flex flex-col items-center justify-center p-2 rounded-lg transition-colors", location.pathname === item.path ? "text-western-parchment bg-western-accent/30" : "text-western-sand hover:text-western-parchment")}>
                    {item.icon}
                    <span className="text-xs mt-1 font-western">{item.label}</span>
                  </Link>)}
                
                {isConnected ? <Link to="/profile" className={cn("flex flex-col items-center justify-center p-2 rounded-lg transition-colors", location.pathname === "/profile" ? "text-western-parchment bg-western-accent/30" : "text-western-sand hover:text-western-parchment")}>
                    <User className="h-4 w-4" />
                    <span className="text-xs mt-1 font-western">Profile</span>
                  </Link> : <Button variant="ghost" size="sm" onClick={handleConnectClick} disabled={connecting} className="flex flex-col items-center justify-center p-2 h-auto text-western-sand hover:text-western-parchment">
