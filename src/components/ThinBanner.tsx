...
export const ThinBanner = () => {
  ...
  return (
    <div className={cn(
      "fixed top-0 left-0 right-0 z-50 py-1 border-b transition-colors", 
      ...
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div>
          ...
          >
            {theme === "dark" ? (
              <Sun className="h-3 w-3 text-western-accent" />
            ) : (
              <Moon className="h-3 w-3 text-western-leather" />
            )}
          </Toggle>
        </div>
        ...
        <div>
          {!isConnected && (
            <Button 
              ...
              disabled={connecting}
              className="h-6 px-2 text-xs border border-western-wood/30 dark:border-western-accent/30"
            >
              <Wallet className="h-3 w-3" />
            </Button>
          )}
          {isConnected && (
            <Wallet className="h-3 w-3 text-western-accent" />
          )}
        </div>
      </div>
    </div>
